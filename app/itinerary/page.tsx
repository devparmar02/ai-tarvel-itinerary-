...
function ItineraryView({ itinerary }: { itinerary: GeneratedItinerary }): React.ReactElement {
  ...
  const { activeDay, setActiveDay, showMap, toggleMap, showBudget, toggleBudget, quizAnswers, personality, savedId, shareToken, setSavedId, resetQuiz, clearPersistedState } = useTravelStore();
...
export default function ItineraryPage() {
  ...
  useEffect(() => {
    if (!selectedDest || isGenerating) return;
    // Allow re-generation if destination changed or no itinerary exists
    if (!itinerary || selectedDest.name !== itinerary.destination) {
  ...
  async function generateItinerary(destination: string) {
    ...
    setGenerating(true);
    setError(null);
    // Reset auto-generated flag to allow new generations
