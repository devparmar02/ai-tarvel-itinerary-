...
function buildPrompt(req: GenerateItineraryRequest): string {
  ...
  return `Craft a ${req.days}-day itinerary for a real person. Every word must earn its place.
...
Return ONLY this exact JSON structure with all ${req.days} days fully filled:
{
  ...
  "days": [
    {
      "day": 1,
      "title": "Evocative story-like title (NOT 'Day 1: Arrival')",
      "morning":   { "activity": "specific vivid activity", "location": "exact place + orientation", "duration_minutes": 90,  "why_it_fits": "2 sentences for this traveller", "best_time": "Early morning", "distance_km": 0 },
      "afternoon": { "activity": "different category from morning", "location": "exact place + orientation", "duration_minutes": 150, "why_it_fits": "2 sentences", "best_time": "Daytime", "distance_km": 3 },
      "evening":   { "activity": "evening-appropriate activity", "location": "exact place + orientation", "duration_minutes": 90,  "why_it_fits": "2 sentences", "best_time": "Sunset", "distance_km": 2 },
...
function fallbackItinerary(req: GenerateItineraryRequest): GeneratedItinerary {
  ...
  const days = Array.from({ length: req.days }, (_, i) => ({
    ...
    morning:    { activity: 'Explore the old quarters on foot', location: req.destination, duration_minutes: 120, why_it_fits: 'Morning light is the best for exploration.',  best_time: 'Early morning', distance_km: 0 },
    afternoon:  { activity: 'Local market and street food walk', location: req.destination, duration_minutes: 120, why_it_fits: 'Markets reveal the real rhythm of a place.',   best_time: 'Daytime',       distance_km: 2 },
    evening:    { activity: 'Sunset viewpoint and dinner',       location: req.destination, duration_minutes: 90,  why_it_fits: 'Every town has a perfect sunset spot.',      best_time: 'Sunset',        distance_km: 1 },
